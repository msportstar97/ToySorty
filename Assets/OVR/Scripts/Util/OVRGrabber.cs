...
public class OVRGrabber : MonoBehaviour
{
    ...
    protected virtual void MoveGrabbedObject(Vector3 pos, Quaternion rot, bool forceTeleport = false)
    {
        ...
        HeadScript hs = m_grabbedObj.GetComponent<HeadScript>();
        GameObject head;
        bool rotateOnly = false;
        if(hs != null)
        {
            rotateOnly = hs.attached;
        }
        ...
        else
        {
            ...
            //head = m_grabbedObj.transform.gameObject;
            //Vector3 old_up = head.transform.up;
            //grabbedRigidbody.MoveRotation(grabbableRotation);
            //Quaternion fixRotation = new Quaternion();
            //fixRotation.SetFromToRotation(head.transform.up, old_up);
            //head.transform.rotation *= fixRotation;

            //head.transform.RotateAround(head.transform.position, head.transform.up, 5f);
            //head.transform.Rotate(head.transform.up, 3f);
            //head.transform.RotateAround(Vector3.zero, head.transform.up, 1f);
            //head.transform.up = hs.body.transform.right;
        }
    }
